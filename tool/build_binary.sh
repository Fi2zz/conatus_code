...
#
# 默认产出 <包根>/dist/conatio，可用第一个参数覆盖输出路径；脚本不依赖调用时的
# cwd：
#   bash tool/build_binary.sh                # -> <包根>/dist/conatio
#   bash tool/build_binary.sh /tmp/conatio   # -> /tmp/conatio
#
...
output="${1:-$package_root/dist/conatio}"
