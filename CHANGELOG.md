...
- checkpoint 存储改为**单文件压缩归档 + 不可读文件名**：每个检查点是一个 `.gz`
  文件（文件名 = `sha256("<会话>:<轮次>")` 前 16 位哈希，**不可读、不暴露轮次
  时间线**；轮次只记录在归档头与会话级 gzip `index` 里，`list`/`prune` 只读索引
  不逐个解包），整个检查点打包成一个文件、不保留目录结构、内容非明文
  （`dart:io` 内置 GZipCodec 自定义容器格式，零新依赖）；旧版目录树检查点
  读时兼容、索引缺失自动重建。
